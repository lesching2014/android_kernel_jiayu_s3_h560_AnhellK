	...
	if (mAudioInit == false) {
		...
		for (i = 0; i <= Soc_Aud_Digital_Block_MEM_HDMI; i++) {
			AFE_Mem_Control_context[i] =
				kzalloc(sizeof(AFE_MEM_CONTROL_T), GFP_KERNEL);
...
{
	pr_warn("%s APLLUsage = %d APLLCounter = %d SampleRate = %d\n", __func__, APLLUsage,
...
uint32 SetCLkMclk(uint32 I2snum, uint32 SampleRate)
{
	...
	pr_warn("%s I2snum = %d I2s_ck_div = %d I2S_APll = %d\n", __func__, I2snum, I2s_ck_div,
		I2S_APll);
...
void Auddrv_AWB_Interrupt_Handler(void)
{
	...
	if (MaxCopySize) {
		...
		PRINTK_AUD_AWB
		("update read pointer u4DMAReadIdx:0x%x, u4WriteIdx:0x%x,
		pucPhysBufAddr:0x%x mBlock->u4MaxCopySize =0x%x\n",
	...
	PRINTK_AUD_AWB
	("+Auddrv_Handle_Mem_context Hw_Get_bytes:0x%x, HW_Cur_ReadIdx:0x%x,
	u4DMAReadIdx:0x%x, u4WriteIdx:0x%x, pucPhysBufAddr:0x%x
	mBlock->u4MaxCopySize =0x%x mBlock->u4DataRemained = 0x%x\n",
	...
	if (mBlock->u4DataRemained > mBlock->u4BufferSize) {
		pr_warn(" buffer overflow u4DMAReadIdx:%x, u4WriteIdx:%x, u4DataRemained:%x, u4BufferSize:%x\n",
	...
	spin_unlock_irqrestore(&Mem_Block->substream_lock, flags);
	PRINTK_AUD_AWB
	("-Auddrv_Handle_Mem_context u4DMAReadIdx:0x%x, u4WriteIdx:0x%x mBlock->u4DataRemained = 0x%x\n",
	 mBlock->u4DMAReadIdx, mBlock->u4WriteIdx, mBlock->u4DataRemained);
}
...
void Auddrv_DAI_Interrupt_Handler(void)
{
	...
	PRINTK_AUD_DAI
	("Auddrv_DAI_Interrupt_Handler Hw_Get_bytes:0x%x, HW_Cur_ReadIdx:0x%x,
	...
	if (mBlock->u4DataRemained > mBlock->u4BufferSize) {
		PRINTK_AUD_DAI
		("Auddrv_DAI_Interrupt_Handler buffer overflow u4DMAReadIdx:%x,
...
#define MAGIC_NUMBER 0xFFFFFFC0
	...
	PRINTK_AUD_DL1
	("+Auddrv_DL1_Interrupt_Handler ReadIdx:%x WriteIdx:%x,
	DataRemained:%x, Afe_consumed_bytes:%x HW_memory_index = %x\n",
...
#define MAGIC_NUMBER 0xFFFFFFC0
	...
	kal_int32 HW_Cur_ReadIdx = 0;
	AFE_BLOCK_T *Afe_Block =
		&(AFE_Mem_Control_context[Soc_Aud_Digital_Block_MEM_DL2]->rBlock);
	...
	PRINTK_AUD_DL2
	("+Auddrv_DL2_Interrupt_Handler ReadIdx:%x WriteIdx:%x,
	DataRemained:%x, Afe_consumed_bytes:%x HW_memory_index = %x\n",
...
void Auddrv_UL1_Interrupt_Handler(void)
{
	...
	PRINTK_AUD_UL1
	("Auddrv_Handle_Mem_context Hw_Get_bytes:%x, HW_Cur_ReadIdx:%x,
	u4DMAReadIdx:%x, u4WriteIdx:0x%x, pucPhysBufAddr:%x Mem_Block->MemIfNum = %d\n",
	 Hw_Get_bytes, HW_Cur_ReadIdx, mBlock->u4DMAReadIdx, mBlock->u4WriteIdx,
	 mBlock->pucPhysBufAddr, Mem_Block->MemIfNum);
...
void Auddrv_UL2_Interrupt_Handler(void)
{
	...
	PRINTK_AUD_UL2
	("Auddrv_UL2_Interrupt_Handler Hw_Get_bytes:%x, HW_Cur_ReadIdx:%x,
	u4DMAReadIdx:%x, u4WriteIdx:0x%x, pucPhysBufAddr:%x Mem_Block->MemIfNum = %d\n",
	...
	if (mBlock->u4DataRemained > mBlock->u4BufferSize) {
		...
		PRINTK_AUD_UL1
		("Auddrv_UL2_Interrupt_Handler buffer overflow u4DMAReadIdx:%x,
		u4WriteIdx:%x, u4DataRemained:%x, u4BufferSize:%x\n",
