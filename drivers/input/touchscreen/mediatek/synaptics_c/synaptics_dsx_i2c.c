...
u8 data20 = 0x00;
static bool use_0d_button = false;
...
/* Vanzo:songlixin on: Wed, 11 Mar 2015 14:52:59 +0800
static u32 gpwDMABuf_pa = NULL;
 */
static dma_addr_t *gpwDMABuf_pa = NULL;
// End of Vanzo: songlixin
static u8 *gprDMABuf_va = NULL;
/* Vanzo:songlixin on: Wed, 11 Mar 2015 14:53:02 +0800
static u32 gprDMABuf_pa = NULL;
 */
static dma_addr_t *gprDMABuf_pa = NULL;
// End of Vanzo: songlixin
struct i2c_msg *read_msg;
static struct device* g_dev = NULL; 

extern struct tpd_device *tpd; // mtk-tpd device
// for 0D button
...
static ssize_t gtp_gesture_wakeup_show(struct device *dev,struct device_attribute *attr,char *buf)
{
    ...
    num_read_chars = snprintf(buf, PAGE_SIZE, "%d\n", _is_gestures_wakup_enable);

    return num_read_chars;
}

static ssize_t gtp_gesture_wakeup_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
    u8 wr_buf[2];
    ...
    if(buf[0] == '1' && _is_gestures_wakup_enable == 0){
        _is_gestures_wakup_enable = 1;
    }
    if(buf[0] == '0' && _is_gestures_wakup_enable == 1){
        _is_gestures_wakup_enable = 0;
    }

    mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
    return count;
}
/* Vanzo:songlixin on: Wed, 11 Mar 2015 14:53:24 +0800
static DEVICE_ATTR(gesture, (S_IWUGO| S_IRUGO), gtp_gesture_wakeup_show, gtp_gesture_wakeup_store);
 */
#ifdef BUILD_CTS
static DEVICE_ATTR(gesture, (S_IRUGO| S_IRUGO), gtp_gesture_wakeup_show, gtp_gesture_wakeup_store);
#else
static DEVICE_ATTR(gesture, (S_IWUGO| S_IRUGO), gtp_gesture_wakeup_show, gtp_gesture_wakeup_store);
#endif
// End of Vanzo: songlixin
...
/* Vanzo:songlixin on: Wed, 11 Mar 2015 14:53:49 +0800
 * adapt to L
...
exit:
    if(gprDMABuf_va){
/* Vanzo:songlixin on: Wed, 11 Mar 2015 14:54:43 +0800
        dma_free_coherent(NULL, 4096, gprDMABuf_va, gprDMABuf_pa);
 */
        dma_free_coherent(&tpd->dev->dev, 4096, gprDMABuf_va, gprDMABuf_pa);
// End of Vanzo: songlixin
        gprDMABuf_va = NULL;
        gprDMABuf_pa = NULL;
    }
...
exit:
    ...
    if(gpwDMABuf_va){
/* Vanzo:songlixin on: Wed, 11 Mar 2015 14:54:19 +0800
        dma_free_coherent(NULL, 1024, gpwDMABuf_va, gpwDMABuf_pa);
 */
        dma_free_coherent(&tpd->dev->dev, 1024, gpwDMABuf_va, gpwDMABuf_pa);
// End of Vanzo: songlixin
        gpwDMABuf_va = NULL;
        gpwDMABuf_pa = NULL;
    }
        ...
        struct synaptics_rmi4_fn *fhandler)
{
    ...
    {   
        ...
        input_report_key(rmi4_data->input_dev, key_value, 1);
        input_sync(rmi4_data->input_dev);
        input_report_key(rmi4_data->input_dev, key_value, 0);
        input_sync(rmi4_data->input_dev);
        return 0;
    }
#endif
    ...
    for (finger = 0; finger < fingers_supported; finger++) {
        ...
        if (finger_status) {
            ...
            if(y>1280)
            {
                ...
                input_report_abs(tpd->dev,ABS_MT_TOUCH_MAJOR, 5);
                input_report_abs(tpd->dev,ABS_MT_POSITION_X, x);
                input_report_abs(tpd->dev,ABS_MT_POSITION_Y, y);

                input_report_abs(tpd->dev, ABS_MT_TRACKING_ID, finger);
        ...
        struct synaptics_rmi4_fn *fhandler)
{
    ...
    {   
       ...
       if(data6[0] == 0x03)
        ...
        input_sync(rmi4_data->input_dev);
        input_report_key(rmi4_data->input_dev, KEY_U, 0);
        input_sync(rmi4_data->input_dev);
       	}
        return 0;
    }
        ...
        struct synaptics_rmi4_fn *fhandler)
{
    ...
    for (button = 0; button < f1a->valid_button_count; button++) {
        ...
        if (rmi4_data->fingers_on_2d == false) {
            ...
            touch_count++;
            /* Vanzo:maxiaojun on: Wed, 23 Jul 2014 11:23:12 +0800
               input_report_key(rmi4_data->input_dev,
             */
            input_report_key(rmi4_data->input_dev,
            ...
            if (before_2d_status[button] == 1) {
                before_2d_status[button] = 0;
                touch_count++;
                /* Vanzo:maxiaojun on: Wed, 23 Jul 2014 11:23:23 +0800
                   input_report_key(rmi4_data->input_dev,
                 */
                input_report_key(rmi4_data->input_dev,
                        // End of Vanzo:maxiaojun
        ...
        bool enable)
{
    ...
    if (enable) {
        ...
        if (!rmi4_data->irq_enabled) {
            ...
            //mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_EN, CUST_EINT_POLARITY_LOW, tpd_eint_handler, 0);
        ...
        const struct i2c_device_id *dev_id)
{
    ...
    {
        input_set_capability(rmi4_data->input_dev, EV_KEY, tpd_gestrue_keys[attr_count]);
    }
...
{   
	...
	else if(data20==0x32)
	{
		...
		{	
			...
			synaptics_rmi4_i2c_write(rmi4_data,0x20,&data5,1);
			_is_enter_gestures_mode = false;
			return;
		}
	}
...
static struct i2c_board_info i2c_tpd={ I2C_BOARD_INFO("synaptics-tpd", (TPD_I2C_ADDR))};
