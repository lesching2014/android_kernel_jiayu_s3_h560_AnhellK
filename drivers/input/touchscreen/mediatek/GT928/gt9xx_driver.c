...
#ifndef GTP_MULTI_CFG
#define GTP_MULTI_CFG   0
#endif
...
static s32 gtp_init_panel(struct i2c_client *client)
{
    ...
    else
    {
        ...
        #if GTP_MULTI_CFG
        {
            u8 gtp_config_mapping_tb[4] = GTP_CONFIG_MAPPING;
            //Need to check SENSOR ID when use 2nd solution TP
            rd_cfg_buf[GTP_ADDR_LENGTH] = gtp_config_mapping_tb[rd_cfg_buf[GTP_ADDR_LENGTH]];//SENSOR ID=2 => 0(GROUP1); SENSOR ID=3 => 1(GROUP2)
        }
        #endif    
    }

    GTP_INFO("CONFIG INDEX:%d", rd_cfg_buf[GTP_ADDR_LENGTH]);
