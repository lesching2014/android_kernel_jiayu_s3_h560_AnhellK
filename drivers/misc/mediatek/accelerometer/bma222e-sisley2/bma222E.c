...
static int BMA222_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	{
/*lenovo-sw molg1 add for Gsesor calibration in EngineerMode begin*/	
		//GSE_LOG("BMA222_ReadSensorData %d,%d,%d!\n", obj->data[BMA222_AXIS_X], obj->data[BMA222_AXIS_Y], obj->data[BMA222_AXIS_Z]);
		acc[BMA222_AXIS_X] = obj->data[BMA222_AXIS_Y] + obj->gs_cali[BMA222_AXIS_X];
		acc[BMA222_AXIS_Y] = (-1)*obj->data[BMA222_AXIS_X] + obj->gs_cali[BMA222_AXIS_Y];
		acc[BMA222_AXIS_Z] = obj->data[BMA222_AXIS_Z] + obj->gs_cali[BMA222_AXIS_Z];
		//GSE_LOG("BMA222_ReadSensorData before xyz: %d,%d,%d!\n", acc[BMA222_AXIS_X], acc[BMA222_AXIS_Y],acc[BMA222_AXIS_Z]);	
		...
		//GSE_LOG("BMA222_ReadSensorData sensitivity:%d xyz: %d,%d,%d!\n", obj->reso->sensitivity, acc[BMA222_AXIS_X], acc[BMA222_AXIS_Y],acc[BMA222_AXIS_Z]);
/*lenovo-sw molg1 add for Gsesor calibration in EngineerMode end*/	
...
static int BMA222_ReadRawData(struct i2c_client *client, char *buf)
{
	...
	{
		sprintf(buf, "BMA222_ReadRawData %04x %04x %04x", obj->data[BMA222_AXIS_X], obj->data[BMA222_AXIS_Y], obj->data[BMA222_AXIS_Z]);
	}
...
static ssize_t show_gs_cali_x_value(struct device_driver *ddri, char *buf)
{
	...
	mutex_lock(&gsensor_mutex);
	cali_x =obj->gs_cali[BMA222_AXIS_X];
	cali_y =obj->gs_cali[BMA222_AXIS_Y];
	cali_z =obj->gs_cali[BMA222_AXIS_Z];
	mutex_unlock(&gsensor_mutex);
...
static ssize_t store_gs_cali_x_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		mutex_lock(&gsensor_mutex);
		obj->gs_cali[BMA222_AXIS_X] = temp;
		mutex_unlock(&gsensor_mutex);
	}
	else
	{
...
static ssize_t store_gs_cali_y_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		mutex_lock(&gsensor_mutex);
		obj->gs_cali[BMA222_AXIS_Y] = temp;
		mutex_unlock(&gsensor_mutex);
	}
	else
	{
		GSE_ERR("store_gs_cali_y_value invalid format\n");
	}
	return count;
}
static ssize_t store_gs_cali_z_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		mutex_lock(&gsensor_mutex);
		obj->gs_cali[BMA222_AXIS_Z] = temp;
		mutex_unlock(&gsensor_mutex);
	}
	else
	{
		GSE_ERR("store_gs_cali_z_value invalid format\n");
	}
	return count;
}
/*----------------------------------------------------------------------------*/
...
static struct driver_attribute *bma222_attr_list[] = {
	...
	&driver_attr_gs_cali_x,         /*show calibration data*/
	&driver_attr_gs_cali_y,         /*show calibration data*/
	&driver_attr_gs_cali_z,         /*show calibration data*/
...
static int bma222_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int err = 0;
	int retry = 0;
	u8 chip_id = 0;	// lenovo-sw youwc1 20150112: bma chip id
	GSE_FUN();
	...
	/* lenovo-sw youwc1 20150112: check chip id to ensure bma222 start */
	err = bma_i2c_read_block(client, BMA_CHIP_ID_REG, &chip_id, 0x01);
	printk("%s - err=%d \n", __func__, err);
	if (err != 0 || 0xF8 != chip_id) 
	{
		printk("%s - chip id=%u \n", __func__, chip_id);
		err = -ENOMEM;
		goto exit_kfree;
	...
	printk("%s - goes here \n", __func__);
	/* lenovo-sw youwc1 20150112: check chip id to ensure bma222 end */
