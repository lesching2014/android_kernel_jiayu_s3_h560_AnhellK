...
static struct acc_init_info LSM6DS0_ACC_init_info = {
        .name = "LSM6DS0_ACC",
        .init = LSM6DS0_ACC_local_init,
        .uninit = LSM6DS0_ACC_remove,
};
...
#ifdef CONFIG_COMPAT
static long lsm6ds0_acc_compat_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
    long err = 0;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = lsm6ds0_acc_compat_ioctl,
#endif
};
/*----------------------------------------------------------------------------*/
...
static int LSM6DS0_ACC_open_report_data(int open)
{
...
static int LSM6DS0_ACC_enable_nodata(int en)
{
    int res =0;
    int retry = 0;
    bool power=false;
    ...
    for(retry = 0; retry < 3; retry++){
        res = LSM6DS0_ACC_SetPowerMode(obj_i2c_data->client, power);
        if(res == 0)
        {
            GSE_LOG("LSM6DS0_ACC_SetPowerMode done\n");
            break;
        }
        GSE_LOG("LSM6DS0_ACC_SetPowerMode fail\n");
    }
    ...
    GSE_LOG("LSM6DS0_ACC_enable_nodata OK!\n");
    return 0;
}

static int LSM6DS0_ACC_set_delay(u64 ns)
{
    int value =0;
    int sample_delay=0;
    int err;
    ...
    if(value <= 5)
    {
        sample_delay = LSM6DS0_ACC_BW_200HZ;
    }
    else if(value <= 10)
    {
        sample_delay = LSM6DS0_ACC_BW_100HZ;
    ...
    err = LSM6DS0_ACC_SetBWRate(obj_i2c_data->client, sample_delay);
    if(err != LSM6DS0_ACC_SUCCESS )
    {
        GSE_ERR("LSM6DS0_set_delay Set delay parameter error!\n");
        return -1;
    }
    GSE_LOG("LSM6DS0_set_delay (%d)\n",value);
    return 0;
}

static int LSM6DS0_ACC_get_data(int* x ,int* y,int* z, int* status)
{
    char buff[LSM6DS0_ACC_BUFSIZE];
    LSM6DS0_ACC_ReadSensorData(obj_i2c_data->client, buff, LSM6DS0_ACC_BUFSIZE);
...
static int LSM6DS0_ACC_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->hw = lsm6ds0_get_cust_acc_hw();
    ...
    if ((err = LSM6DS0_ACC_create_attr(&(LSM6DS0_ACC_init_info.platform_diver_addr->driver))))
	{
		GSE_ERR("create attribute err = %d\n", err);
		goto exit_create_attr_failed;
	}

    ctl.open_report_data= LSM6DS0_ACC_open_report_data;
    ctl.enable_nodata = LSM6DS0_ACC_enable_nodata;
    ctl.set_delay  = LSM6DS0_ACC_set_delay;
    ...
    data.get_data = LSM6DS0_ACC_get_data;
    ...
    err = batch_register_support_info(ID_ACCELEROMETER,obj->hw->is_batch_supported,1000, 0);
    if(err)
    {
        GSE_ERR("register acc batch support err = %d\n", err);
        goto exit_kfree;
    }
...
static int LSM6DS0_ACC_i2c_remove(struct i2c_client *client)
{
    ...
    if ((err =  LSM6DS0_ACC_delete_attr(&(LSM6DS0_ACC_init_info.platform_diver_addr->driver))))
	{
...
{
	struct acc_hw *hw = lsm6ds0_get_cust_acc_hw();
	GSE_FUN();
...
static int __init LSM6DS0_ACC_init(void)
{
    ...
    acc_driver_add(&LSM6DS0_ACC_init_info);
	return 0;    
}
/*----------------------------------------------------------------------------*/
