...
static struct i2c_board_info k2dh_i2c_info __initdata = {
	I2C_BOARD_INFO(K2DH_ACC_DEV_NAME, K2DH_ACC_I2C_SAD_L)
};
...
/*----------------------------------------------------------------------------*/
static struct i2c_client *k2dh_i2c_client;
static struct platform_driver k2dh_gsensor_driver;
static struct k2dh_i2c_data *obj_i2c_data;
static bool sensor_power = true;
static int test_status;
static UINT32 data_read_count;
static GSENSOR_VECTOR3D gsensor_gain;

/*----------------------------------------------------------------------------*/
#define GSE_TAG                  "[Gsensor] "
#define GSE_FUN(f)               pr_debug("[Gsensor] %s()\n", __func__)
#define GSE_ERR(fmt, args...)    pr_debug("[Gsensor] [ERROR] %s() line=%d : "fmt, __func__, __LINE__, ##args)
...
static int k2dh_acc_register_write(struct i2c_client *client, u8 *buf, u8 reg_address,
				   u8 new_value)
{
	int err = -1;
...
static int k2dh_acc_register_update(struct i2c_client *client, u8 *buf, u8 reg_address, u8 mask,
				    u8 new_bit_values)
{
	int err = -1;
	u8 init_val;
...
static int k2dh_ReadData(struct i2c_client *client, s16 data[K2DH_AXES_NUM])
{
	...
	err = k2dh_acc_i2c_read(client, acc_data, K2DH_DATA_LEN);
	if ((err) != K2DH_SUCCESS) {
		...
		if ((prev_data[K2DH_AXIS_X] == data[K2DH_AXIS_X])
			...
			if (check_cnt >= READ_DATA_CHECK_CNT) {	/* read data same case */
				GSE_ERR("READ DATA SAME CASE : (%d %d %d)\n", data[K2DH_AXIS_X],
					data[K2DH_AXIS_Y], data[K2DH_AXIS_Z]);
		...
		if (atomic_read(&priv->filter)) {
			if (atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend)) {
				...
				if (priv->fir.num < firlen) {
					priv->fir.raw[priv->fir.num][K2DH_AXIS_X] =
					    data[K2DH_AXIS_X];
					priv->fir.raw[priv->fir.num][K2DH_AXIS_Y] =
					    data[K2DH_AXIS_Y];
					priv->fir.raw[priv->fir.num][K2DH_AXIS_Z] =
					    data[K2DH_AXIS_Z];
					...
					if (atomic_read(&priv->trace) & K2DH_TRC_FILTER) {
						GSE_LOG
						    ("add [%2d] [%5d %5d %5d] => [%5d %5d %5d]\n",
						     ...
						     priv->fir.raw[priv->fir.num][K2DH_AXIS_Y],
						     priv->fir.raw[priv->fir.num][K2DH_AXIS_Z],
						     priv->fir.sum[K2DH_AXIS_X],
						     priv->fir.sum[K2DH_AXIS_Y],
						     priv->fir.sum[K2DH_AXIS_Z]);
					}
					priv->fir.num++;
					priv->fir.idx++;
				} else {
					...
					priv->fir.sum[K2DH_AXIS_X] -=
					    priv->fir.raw[idx][K2DH_AXIS_X];
					priv->fir.sum[K2DH_AXIS_Y] -=
					    priv->fir.raw[idx][K2DH_AXIS_Y];
					priv->fir.sum[K2DH_AXIS_Z] -=
					    priv->fir.raw[idx][K2DH_AXIS_Z];
					...
					if (atomic_read(&priv->trace) & K2DH_TRC_FILTER) {
						GSE_LOG
						    ("add [%2d] [%5d %5d %5d] => [%5d %5d %5d] : [%5d %5d %5d]\n",
						     idx, priv->fir.raw[idx][K2DH_AXIS_X],
						     priv->fir.raw[idx][K2DH_AXIS_Y],
						     priv->fir.raw[idx][K2DH_AXIS_Z],
						     priv->fir.sum[K2DH_AXIS_X],
						     priv->fir.sum[K2DH_AXIS_Y],
						     priv->fir.sum[K2DH_AXIS_Z], data[K2DH_AXIS_X],
						     data[K2DH_AXIS_Y], data[K2DH_AXIS_Z]);
					}
				}
			}
...
static int k2dh_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	res = k2dh_ReadData(client, obj->data);
	if (res) {
		...
		/*remap coordinate */
		acc[obj->cvt.map[K2DH_AXIS_X]] =
		    obj->cvt.sign[K2DH_AXIS_X] * obj->data[K2DH_AXIS_X];
		acc[obj->cvt.map[K2DH_AXIS_Y]] =
		    obj->cvt.sign[K2DH_AXIS_Y] * obj->data[K2DH_AXIS_Y];
		acc[obj->cvt.map[K2DH_AXIS_Z]] =
		    obj->cvt.sign[K2DH_AXIS_Z] * obj->data[K2DH_AXIS_Z];
		...
		sprintf(buf, "%04x %04x %04x %x", acc[K2DH_AXIS_X], acc[K2DH_AXIS_Y],
			acc[K2DH_AXIS_Z], data_read_count);
...
static int k2dh_do_calibration(void)
{
	...
	if ((abs(sum[K2DH_AXIS_X] / CALIBRATION_DATA_AMOUNT) > TESTLIMIT_XY) ||
	    ...
	    ((abs(sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) > TESTLIMIT_Z_USL_LSB)
	     || (abs(sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) < TESTLIMIT_Z_LSL_LSB))) {
		GSE_LOG("Calibration zero-g offset check failed (%d, %d, %d)\n",
			sum[K2DH_AXIS_X] / CALIBRATION_DATA_AMOUNT,
			sum[K2DH_AXIS_Y] / CALIBRATION_DATA_AMOUNT,
			sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT);
		atomic_set(&obj->fast_calib_rslt, 0);
		return -EINVAL;
	}
#endif
	...
	if (sum[K2DH_AXIS_Z] < 0) {
		obj->cali_sw[K2DH_AXIS_Z] = (sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) + 1024;
		/* K2DH(12bit) 1024 +- 226 */
	} else {
		obj->cali_sw[K2DH_AXIS_Z] = (sum[K2DH_AXIS_Z] / CALIBRATION_DATA_AMOUNT) - 1024;
		/* K2DH(12bit) 1024 +- 226 */
	...
	GSE_LOG("%s : calibration data (%d, %d, %d)\n", __func__, obj->cali_sw[K2DH_AXIS_X],
		obj->cali_sw[K2DH_AXIS_Y], obj->cali_sw[K2DH_AXIS_Z]);
	...
	if (val >= 0) {
		for (val = 1, i = 0; i < 3; i++) {
			if (i < 2) {
				...
				if ((abs(NO_ST_ZOFF[i]) > TESTLIMIT_Z_USL_LSB)
				    || (abs(NO_ST_ZOFF[i]) < TESTLIMIT_Z_LSL_LSB)) {
					GSE_ERR("[SELFTEST] NO_ST[%d] : Out of ZOffset!! (%d)\n", i,
						NO_ST[i]);
					val = -1;
				}
	...
	/* if(atomic_read(&obj->trace) & K2DH_TRC_CALI) */
	{
		if (val >= 0) {
			GSE_LOG("[SELFTEST] OK!! val : %d, (%d, %d, %d) ||| (%d, %d, %d)\n", val,
				ST[0], ST[1], ST[2], NO_ST_ZOFF[0], NO_ST_ZOFF[1], NO_ST_ZOFF[2]);
		} else {
...
static int k2dh_set_mode(struct i2c_client *client, unsigned char mode)
{
	...
	comres =
	    k2dh_acc_register_update(client, (u8 *) data, K2DH_LP_EN_REG, K2DH_LP_EN_MSK,
				     LPen << K2DH_LP_EN_POS);
	if (comres != K2DH_SUCCESS) {
		return comres;
	} else {
		comres =
		    k2dh_acc_register_update(client, (u8 *) data, K2DH_HR_REG, K2DH_HR_MSK,
					     HR << K2DH_HR_POS);
		if (comres != K2DH_SUCCESS)
			return comres;
...
static int k2dh_set_range(struct i2c_client *client, unsigned char range)
{
	...
	comres =
	    k2dh_acc_register_update(client, (u8 *) data, K2DH_FS_REG, K2DH_FS_MSK,
				     range << K2DH_FS_POS);
	if (comres != K2DH_SUCCESS)
		return comres;
...
static int k2dh_set_bandwidth(struct i2c_client *client, unsigned char bandwidth)
{
	...
	comres =
	    k2dh_acc_register_update(client, (u8 *) data, K2DH_ODR_REG, K2DH_ODR_MSK,
				     bandwidth << K2DH_ODR_POS);
	if (comres != K2DH_SUCCESS)
		return comres;
...
static ssize_t show_status_value(struct device_driver *ddri, char *buf)
{
	...
	if (obj->hw) {
		len +=
		    snprintf(buf + len, PAGE_SIZE - len, "CUST: %d %d (%d %d)\n", obj->hw->i2c_num,
...
static ssize_t k2dh_softreset_store(struct device *dev, struct device_attribute *attr,
...
static DRIVER_ATTR(bandwidth, S_IWUSR | S_IRUGO | S_IWGRP, show_bandwidth_value,
		   store_bandwidth_value);
...
static DRIVER_ATTR(fast_calibration_x, S_IRUGO | S_IWUSR | S_IWGRP, k2dh_fast_calibration_x_show,
		   k2dh_fast_calibration_x_store);
static DRIVER_ATTR(fast_calibration_y, S_IRUGO | S_IWUSR | S_IWGRP, k2dh_fast_calibration_y_show,
		   k2dh_fast_calibration_y_store);
static DRIVER_ATTR(fast_calibration_z, S_IRUGO | S_IWUSR | S_IWGRP, k2dh_fast_calibration_z_show,
		   k2dh_fast_calibration_z_store);
static DRIVER_ATTR(eeprom_writing, S_IRUGO | S_IWUSR | S_IWGRP, k2dh_eeprom_writing_show,
		   k2dh_eeprom_writing_store);

#if 0
static DRIVER_ATTR(run_fast_calibration, S_IRUGO | S_IWUSR | S_IWGRP, k2dh_runCalibration_show,
		   k2dh_runCalibration_store);
#endif
#endif
#ifdef CONFIG_K2DH_LOWPASS
...
static int k2dh_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		err = driver_create_file(driver, k2dh_attr_list[idx]);
		if (err) {
			GSE_ERR("driver_create_file (%s) = %d\n", k2dh_attr_list[idx]->attr.name,
				err);
			break;
		}
	}
...
/*----------------------------------------------------------------------------*/
int gsensor_operate(void *self, uint32_t command, void *buff_in, int size_in, void *buff_out,
...
static int k2dh_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = k2dh_acc_hw_init(new_client, 1);
	if (err) {
		GSE_ERR("failed to init K2DH ( err = %d )\n", err);
		goto exit_init_failed;
	}

	err = misc_register(&k2dh_device);
	if (err) {
		GSE_ERR("k2dh_device register failed\n");
		goto exit_misc_device_register_failed;
	}

	err = k2dh_create_attr(&k2dh_gsensor_driver.driver)
	    if (err) {
	...
	obj->early_drv.level = EARLY_SUSPEND_LEVEL_DISABLE_FB - 1,
	    ...
	    obj->early_drv.resume = k2dh_late_resume, register_early_suspend(&obj->early_drv);
#endif
...
static int k2dh_i2c_remove(struct i2c_client *client)
{
	...
	err = k2dh_delete_attr(&k2dh_gsensor_driver.driver);
	if (err)
		GSE_ERR("k2dh_delete_attr fail: %d\n", err);


	err = misc_deregister(&k2dh_device);
	if (err)
