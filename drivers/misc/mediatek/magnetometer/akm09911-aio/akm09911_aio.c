...
static struct i2c_board_info __initdata i2c_akm09911={ I2C_BOARD_INFO("akm09911", 0x1A >> 1)};
/*----------------------------------------------------------------------------*/
...
static int akm09911_init_flag =-1; // 0<==>OK -1 <==> fail
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{
		printk(KERN_ERR "invalid content: '%s'\n", buf);
	}
...
static struct file_operations akm09911_fops = {
	...
	//.compat_ioctl = akm09911_unlocked_ioctl,
};
/*----------------------------------------------------------------------------*/
...
static void akm09911_early_suspend(struct early_suspend *h) 
{
	...
	int err = 0;
	AKMDBG("akm09911_early_suspend entry!\n");
	if(NULL == obj)
	{
	...
	akm09911_power(obj->hw, 0);       
	AKMDBG("akm09911_early_suspend exit!\n");
}
/*----------------------------------------------------------------------------*/
static void akm09911_late_resume(struct early_suspend *h)
{
	struct akm09911_i2c_data *obj = container_of(h, struct akm09911_i2c_data, early_drv);         
	int err;
	AKMDBG("akm09911_late_resume entry!\n");
	if(NULL == obj)
	{
		printk(KERN_ERR "null pointer!!\n");
		return;
	...
	AKMDBG("akm09911_late_resume exit!\n");
}
/*----------------------------------------------------------------------------*/
...
static int akm09911_m_set_delay(u64 ns)
{
	...
	if(value <= AKM09911_MIN_DELAY)
    {
        akmd_delay = AKM09911_MIN_DELAY;
    }
    else{
        akmd_delay = value;
	}
...
#ifdef AKM_Pseudogyro
static int akm09911_gs_enable(int en)
{
	int value = 0;
	value = en;
...
static int akm09911_gs_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
	if(value <= AKM09911_MIN_DELAY)
...
static int akm09911_gs_open_report_data(int open)
{
	return 0;
}

static int akm09911_gs_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int akm09911_rv_enable(int en)
{
	int value = 0;
	value = en;
	factory_mode = 1;
...
static int akm09911_rv_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
...
static int akm09911_rv_open_report_data(int open)
{
	return 0;
}

static int akm09911_rv_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int akm09911_gr_enable(int en)
{
	int value = 0;
	value = en;
...
static int akm09911_gr_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
	if(value <= AKM09911_MIN_DELAY)
...
static int akm09911_gr_open_report_data(int open)
{
	return 0;
}

static int akm09911_gr_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
static int akm09911_la_enable(int en)
{
	int value = 0;
	value = en;
...
static int akm09911_la_set_delay(u64 ns)
{
	int value = 0;
	value = (int)ns/1000/1000;
	if(value <= AKM09911_MIN_DELAY)
...
static int akm09911_la_open_report_data(int open)
{
	return 0;
}

static int akm09911_la_get_data(int* x ,int* y,int* z, int* status)
{
	mutex_lock(&sensor_data_mutex);
...
//	struct hwmsen_object sobj_gravity, sobj_la;
	...
	AKMDBG("akm09911_i2c_probe entry!!!\n");
	...
	AKMDBG("akm09911_i2c_probe: befor init prob \n");
	/* Check connection */
	err = AKECS_CheckDevice();
	if(err < 0)
	{
		AKMDBG("akm09911_i2c_probe: check device connect error:%d\n", err);
	...
	{
		AKMDBG("akm09911_device register failed\n");
...
#ifdef AKM_Pseudogyro
		ctl.gs_enable = akm09911_gs_enable;
		ctl.gs_set_delay  = akm09911_gs_set_delay;
		ctl.gs_open_report_data = akm09911_gs_open_report_data;
		ctl.rv_enable = akm09911_rv_enable;
		ctl.rv_set_delay  = akm09911_rv_set_delay;
		ctl.rv_open_report_data = akm09911_rv_open_report_data;
		ctl.gr_enable = akm09911_gr_enable;
		ctl.gr_set_delay  = akm09911_gr_set_delay;
		ctl.gr_open_report_data = akm09911_gr_open_report_data;
		ctl.la_enable = akm09911_la_enable;
		ctl.la_set_delay  = akm09911_la_set_delay;
		ctl.la_open_report_data = akm09911_la_open_report_data;
#endif
...
#ifdef AKM_Pseudogyro
	    mag_data.get_data_gs = akm09911_gs_get_data;
		mag_data.get_data_rv = akm09911_rv_get_data;
		mag_data.get_data_gr = akm09911_gr_get_data;
		mag_data.get_data_la = akm09911_la_get_data;
#endif
...
static int akm09911_local_init(void)
{
	...
	{
		AKMDBG("akm09911_local_init error\n");
		return -1;
	}
