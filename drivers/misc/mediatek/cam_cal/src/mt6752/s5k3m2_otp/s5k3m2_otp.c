...
#define CAM_CALDB(fmt, arg...)    xlog_printk(ANDROID_LOG_DEBUG   , PFX, "[%s] " fmt, __func__, ##arg)
#define CAM_CALERR(fmt, arg...)    xlog_printk(ANDROID_LOG_ERROR   , PFX, "[%s] " fmt, __func__, ##arg)
#else
#define CAM_CALDB(x,...)
#endif
...
static int Read3M2AWBData(kal_uint16 ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	unsigned int sum = 0;
...
static int Read3M2AFData(kal_uint16 ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	unsigned int sum = 0;
...
static int Read3M2LSCData(kal_uint16 ui4_offset, unsigned int  ui4_length, unsigned char * pinputdata)
{
	int i = 0;
	int addr = ui4_offset;
	unsigned int sum = 0;
...
//   .detect = CAM_CAL_i2c_detect,
    .driver.name = S5K3M2_CAM_CAL_DRVNAME,
