...
#define STP_INFO_FUNC(fmt, arg...)   if(gStpDbgLvl >= STP_LOG_INFO){ osal_info_print(PFX "%s:[I] "  fmt, __func__ ,##arg);}
#define STP_WARN_FUNC(fmt, arg...)   if(gStpDbgLvl >= STP_LOG_WARN){ osal_warn_print(PFX "%s:[W] "  fmt, __func__ ,##arg);}
#define STP_ERR_FUNC(fmt, arg...)    if(gStpDbgLvl >= STP_LOG_ERR){  osal_err_print(PFX "%s:[E] "   fmt, __func__ ,##arg);}
