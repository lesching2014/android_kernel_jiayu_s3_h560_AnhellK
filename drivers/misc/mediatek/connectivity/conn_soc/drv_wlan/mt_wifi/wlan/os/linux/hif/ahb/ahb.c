...
VOID glSetHifInfo(GLUE_INFO_T *GlueInfo, ULONG ulCookie)
{
	...
	HifInfo->WIFIRegBaseAddr = ioremap(WIFI_CR_REG_BASE, WIFI_CR_REG_LENGTH);

	DBGLOG(INIT, INFO,
		"[WiFi/HIF]HifInfo->HifRegBaseAddr=0x%p, HifInfo->McuRegBaseAddr=0x%p, HifInfo->WIFIRegBaseAddr=0x%p\n",
		HifInfo->HifRegBaseAddr, HifInfo->McuRegBaseAddr, HifInfo->WIFIRegBaseAddr);
...
VOID glClearHifInfo(GLUE_INFO_T *GlueInfo)
{
	...
	iounmap(GlueInfo->rHifInfo.WIFIRegBaseAddr);
	return;
