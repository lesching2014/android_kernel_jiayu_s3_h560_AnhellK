...
#define mtk_cooler_mdulthro_dprintk_always(fmt, args...) \
	do { pr_debug("thermal/cooler/mdulthro" fmt, ##args); } while (0)

#define mtk_cooler_mdulthro_dprintk(fmt, args...) \
	do { \
		if (1 == cl_mdulthro_klog_on) { \
			pr_debug("thermal/cooler/mdulthro" fmt, ##args); \
		} \
	} while (0)
...
static int wmt_cal_stats(unsigned long data)
{
	...
	if (pre_time != 0 && cur_time.tv_sec > pre_time) {
		...
		if (tx_bytes > stats_info->pre_tx_bytes) {
			...
			tx_throughput =
				((tx_bytes - stats_info->pre_tx_bytes) / (cur_time.tv_sec - pre_time)) >> 7;
			...
			/* Overflow */
			tx_throughput =
				((0xffffffff - stats_info->pre_tx_bytes + tx_bytes)
...
static int wmt_judge_throttling(int index, int is_on, int interval)
{
	...
	mtk_cooler_mdulthro_dprintk("[%s]+ [0]=%d, [1]=%d || [%d] is %s\n",
		__func__, mail_box[0], mail_box[1], index, (is_on == 1 ? "ON" : "OFF"));
		...
		switch(cur_wifi_stat) {
			...
			case HIGH_STAT:
				if (throttling_pre_stat < HIGH_STAT || throttling_pre_stat == WFD_STAT) {
					...
					mtk_cooler_mdulthro_dprintk_always("LOW/MID-->HIGH:%lu <- (%d / %d) %lu",
					...
					*/
					...
					if ( (over_up_time * interval) >= up_duration) {
						...
						mtk_cooler_mdulthro_dprintk_always("HIGH-->HIGH:%lu <- (%d / %d) %lu",
							thro_constraint, up_numerator, up_denominator, cur_thro);
			...
			case LOW_STAT:
				if (throttling_pre_stat == WFD_STAT) {
					...
					mtk_cooler_mdulthro_dprintk_always("MID/HIGH-->LOW:%lu <- (%d / %d) %lu",
					...
					*/
					...
					if ( (below_low_time*interval) >= low_duration) {
						if (low_rst_time >= low_rst_max && !is_reset) {
							...
							mtk_cooler_mdulthro_dprintk_always(
								"LOW-->LOW:%lu <-(%d / %d) %lu",
...
#if 0
static int _mtk_cl_mdulthro_proc_read(char *buf, char **start,
...
int md_tx_thro_read(char *buf, char **start, off_t offset, int count,
	int *eof, void *data)
{
...
/*New Wifi throttling Algo+*/
int mdulthro_param_read(char *buf, char **start, off_t offset, int count,
	int *eof, void *data)
{
	int ret;
	char tmp[MAX_LEN] = {0};
...
ssize_t mdulthro_param_write(struct file *filp, const char __user *buf,
	unsigned long len, void *data)
{
	...
	if (sscanf(desc, "%d %d/%d %d %d/%d %d %d",
		...
		mtk_cooler_mdulthro_dprintk_always("[%s] %s [up]%d %d/%d, [low]%d %d/%d, rst=%d, interval\n",
			__func__, desc, up_duration, up_numerator, up_denominator,
			low_duration, low_numerator, low_denominator, low_rst_max, polling_interval);
...
int mdulthro_pid_read(char *buf, char **start, off_t offset , int count,
	int *eof, void *data)
{
	int ret;
	char tmp[MAX_LEN] = {0};
...
ssize_t mdulthro_pid_write(struct file *filp, const char __user *buf,
	unsigned long len, void *data)
{
	int ret = 0;
	char tmp[MAX_LEN] = {0};
