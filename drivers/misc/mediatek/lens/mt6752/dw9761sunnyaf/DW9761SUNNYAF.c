...
#define PLATFORM_DRIVER_NAME "lens_actuator_dw9761sunnyaf"
#define AF_DRIVER_CLASS_NAME "actuatordrv_dw9761sunnyaf"
...
inline static int getAFInfo(__user stDW9761SUNNYAF_MotorInfo * pstMotorInfo)
{
    stDW9761SUNNYAF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stDW9761SUNNYAF_MotorInfo)))
    {
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case DW9761SUNNYAFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stDW9761SUNNYAF_MotorInfo *)(a_u4Param));
        break;

        case DW9761SUNNYAFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;
 
        case DW9761SUNNYAFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case DW9761SUNNYAFIOC_T_SETMACROPOS :
...
static int __init DW9761OFILMAF_i2C_init(void)
{
...
static void __exit DW9761OFILMAF_i2C_exit(void)
{
	platform_driver_unregister(&g_stAF_Driver);
}

module_init(DW9761OFILMAF_i2C_init);
module_exit(DW9761OFILMAF_i2C_exit);

MODULE_DESCRIPTION("DW9761OFILMAF lens module driver");
