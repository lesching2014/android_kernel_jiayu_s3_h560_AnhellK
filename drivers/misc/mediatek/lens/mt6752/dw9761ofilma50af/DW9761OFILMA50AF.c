...
#define AF_DRVNAME "DW9761OFILMA50AF"
#define I2C_SLAVE_ADDRESS        0x18
#define I2C_REGISTER_ID            0x1a //lenovo.sw wangsx3 change the i2c id for different i2c_baord_info
#define PLATFORM_DRIVER_NAME "lens_actuator_dw9761ofilma50af"
#define AF_DRIVER_CLASS_NAME "actuatordrv_dw9761ofilma50af"

static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO(AF_DRVNAME, I2C_REGISTER_ID)};
#define AF_DEBUG
#ifdef AF_DEBUG
#define LOG_INF(format, args...) xlog_printk(ANDROID_LOG_INFO,    AF_DRVNAME, "[%s] " format, __func__, ##args)
#else
...
{
    int  i4RetValue = 0;
   //lenovo.sw wangsx3  revise dw9761sunnysf driver and open SAC mode
...
inline static int getAFInfo(__user stDW9761OFILMA50AF_MotorInfo * pstMotorInfo)
{
    stDW9761OFILMA50AF_MotorInfo stMotorInfo;
    ...
    if(copy_to_user(pstMotorInfo , &stMotorInfo , sizeof(stDW9761OFILMA50AF_MotorInfo)))
    {
...
unsigned long a_u4Param)
{
    ...
    switch(a_u4Command)
    {
        case DW9761OFILMA50AFIOC_G_MOTORINFO :
            i4RetValue = getAFInfo((__user stDW9761OFILMA50AF_MotorInfo *)(a_u4Param));
        break;

        case DW9761OFILMA50AFIOC_T_MOVETO :
            i4RetValue = moveAF(a_u4Param);
        break;
 
        case DW9761OFILMA50AFIOC_T_SETINFPOS :
            i4RetValue = setAFInf(a_u4Param);
        break;

        case DW9761OFILMA50AFIOC_T_SETMACROPOS :
...
static int AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	char puSendCmd4[2] = {0x07,0x05};//Tvib=13.6ms
...
static int AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //lenovo.sw wangsx3 add for AIO,lens driver compatibility
...
static int __init DW9761SUNNYA50AF_i2C_init(void)
{
...
static void __exit DW9761SUNNYA50AF_i2C_exit(void)
{
	platform_driver_unregister(&g_stAF_Driver);
}

module_init(DW9761SUNNYA50AF_i2C_init);
module_exit(DW9761SUNNYA50AF_i2C_exit);

MODULE_DESCRIPTION("DW9761AFA50 lens module driver");
