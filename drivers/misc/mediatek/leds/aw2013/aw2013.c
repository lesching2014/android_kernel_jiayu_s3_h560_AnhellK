...
/* #if defined(DCT_M316B) */
#warning "klutu 111"
#define AW2013_I2C_BUSNUM 1
/* #else */
/* #define AW2013_I2C_BUSNUM 0 */
/* #endif */
...
void led_breath_turnon()
{
    switch (breath_num)
    {
      ...
      case AUTO_BREATH:
        RE_IIC_WriteReg(0x01, 0x01);    // enable LED 不使用中断
        RE_IIC_WriteReg(0x31, 0x71);    //config mode, IMAX
        RE_IIC_WriteReg(0x32, 0x71);    //config mode, IMAX
        RE_IIC_WriteReg(0x33, 0x71);    //config mode, IMAX
      ...
      case BRIGHT_STATUS:
        RE_IIC_WriteReg(0x01, 0x01);    // enable LED 不使用中断
        RE_IIC_WriteReg(0x31, 0x61);    //config mode, IMAX
        RE_IIC_WriteReg(0x32, 0x61);    //config mode, IMAX
        RE_IIC_WriteReg(0x33, 0x61);    //config mode, IMAX
