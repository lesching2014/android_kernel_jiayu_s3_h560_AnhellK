...
static kal_uint32 charging_hw_init(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "[charging_hw_init] From Zax\n");
...
#endif
		{
			...
			if (charging_get_error_state()) {
				battery_log(BAT_LOG_CRTI,
					    "[charging_enable] bq24261_set_hz_mode(0x1)\n");
...
#endif
				battery_log(BAT_LOG_CRTI,
					    "[charging_set_cv_voltage] set low CV by 6311 E1\n");
			} else {
				if (pre_register_value != register_value) {
					battery_log(BAT_LOG_CRTI,
						    "[charging_set_cv_voltage] disable charging\n");
...
static kal_uint32 charging_get_battery_status(void *data)
{
	...
	*(kal_bool *) (data) = 0;	/* battery exist */
	...
	battery_log(BAT_LOG_CRTI, "[charging_get_battery_status] BATON_TDET_EN = %d\n", val);
	if (val) {
...
#endif
	battery_log(BAT_LOG_CRTI, " [%s] enable: %d, flag: %d!\n", __func__, enable, flag);
}
...
#if !defined(MTK_AUXADC_IRQ_SUPPORT)
int _get_irq_direction(int pre_vol, int cur_vol, int irq_direct)
{
	int ret = -1;
...
static void _get_polling_state(void)
{
	...
	if (DISO_Polling.reset_polling) {
		...
		if (vdc_vol > (VDC_MIN_VOLTAGE / 1000))
			vdc_vol_dir = DISO_IRQ_RISING;
		else if (vdc_vol < (VDC_MIN_VOLTAGE / 1000))
			vdc_vol_dir = DISO_IRQ_FALLING;

		if (vusb_vol > (VBUS_MIN_VOLTAGE / 1000))
			vusb_vol_dir = DISO_IRQ_RISING;
		else if (vusb_vol < (VBUS_MIN_VOLTAGE / 1000))
			vusb_vol_dir = DISO_IRQ_FALLING;
	} else {
		/* get voltage direction */
		vdc_vol_dir =
		    _get_irq_direction(VDC_Polling->preVoltage, VDC_Polling->curVoltage,
	...
	if (VDC_Polling->notify_irq_en && (vdc_vol_dir == VDC_Polling->notify_irq)) {
		...
		battery_log(BAT_LOG_CRTI,
			    "[%s] ready to trig VDC irq, irq: %d, curVoltage: %d, preVoltage: %d\n",
			    __func__, VDC_Polling->notify_irq, VDC_Polling->curVoltage,
			    VDC_Polling->preVoltage);
		...
		battery_log(BAT_LOG_CRTI,
			    "[%s] ready to trig VUSB irq, irq: %d, curVoltage: %d, preVoltage: %d\n",
			    __func__, VUSB_Polling->notify_irq, VUSB_Polling->curVoltage,
			    VUSB_Polling->preVoltage);
...
int diso_thread_kthread(void *x)
{
	...
	while (1) {
		...
		if (DISO_Polling.vdc_polling_measure.notify_irq_en ||
			...
			battery_log(BAT_LOG_FULL, "[%s] disable diso thread\n", __func__);
			hrtimer_cancel(&diso_kthread_timer);
		}
...
static kal_uint32 charging_get_diso_state(void *data)
{
	...
	} else if (diso_state == 0x0) {
		pDISO_data->diso_state.cur_vusb_state = DISO_OFFLINE;
		pDISO_data->diso_state.cur_vdc_state = DISO_OFFLINE;
		pDISO_data->diso_state.cur_otg_state = DISO_OFFLINE;
		battery_log(BAT_LOG_CRTI, "[%s] usb: %d, dc: %d, otg: %d!\n",
			    __func__, pDISO_data->diso_state.cur_vusb_state,
