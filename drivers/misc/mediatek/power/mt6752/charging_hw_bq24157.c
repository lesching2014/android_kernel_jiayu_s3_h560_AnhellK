...
//		if(charging_led_flag==1)
		{
			bq24157_config_interface(0x00, 0, 0x01, 6);
			charging_led_flag = 0;
		}
...
//		if(charging_led_flag==0)
		{
			bq24157_config_interface(0x00, 1, 0x01, 6);
			charging_led_flag = 1;
		}
...
void lenovo_charging_term_set_stage_2(void)
{
	if(lenovo_charging_stage==1)
	{
		...
		bq24157_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);
	}
}
...
{
	if(lenovo_charging_stage==1)
		bq24157_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_1);//lenovo_charging_set_iterm(CHARGE_CURRENT_200_00_MA);
	else
		bq24157_set_iterm(LENOVO_CHARGING_TERM_CUR_STAGE_2);//lenovo_charging_set_iterm(CHARGE_CURRENT_150_00_MA);
}
#endif
...
#else
   bq24157_config_interface(0x00, 0, 0x01, 7);	//kick chip watch dog
#endif
/*lenovo-sw weiweij added for led control end*/
	...
	if ( !charging_init_flag ) {   
		bq24157_reg_config_interface(0x04,0x1A); //146mA
