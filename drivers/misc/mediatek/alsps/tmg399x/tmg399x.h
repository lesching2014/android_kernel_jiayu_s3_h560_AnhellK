...
enum tmg399x_caloffsetstates {
	START_CALOFF,     //!< state to start offset calibration (transistion state)
	CHECK_PROX_NE,    //!< state to check and adapt proximity NE offset
	CHECK_PROX_SW,    //!< state to check and adapt proximity SW offset
	CHECK_NSWE_ZERO,  //!< state to check and adapt gesture NSWE offsets
	CALOFF_OK         //!< state to indicate that offset calibration has finished successfully
};

enum tmg399x_caloffsetdirections {
	DIR_NONE,       //!< no specific direction set
	DIR_UP,         //!< direction up
