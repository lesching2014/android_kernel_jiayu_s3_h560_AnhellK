...
UINT32 sw_charging_timer_enable = 0;
UINT32 sw_charging_timer = 5 * 60 * 60; //5 hours = 300 mins = 18000 seconds
...
// <<< 2016/05/23-dangyiwang. Battery. add adb command for close Safety timer
kal_bool g_safety_timer_enable_flag = KAL_TRUE;
// >>> 2016/05/23-dangyiwang. Battery. add adb command for close Safety timer
...
static kal_bool g_charging_full_first_flag = KAL_FALSE;  //<//-tedwu, fix the not charged full.//>
//>2015/01/18-tedwu
...
/*[Lavender][bozhi_lin] set AC timer to 5 hours and USB to 100 hours 20150415 begin*/
static ssize_t show_SW_Charging_Timer_Status(struct device *dev,struct device_attribute *attr,
					char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "BMT_status.charging_timer=%d (sw_charging_timer_enable=%d,sw_charging_timer=%d)\n", BMT_status.charging_timer, sw_charging_timer_enable, sw_charging_timer);
    return sprintf(buf, "%d (sw_charging_timer_enable=%d,sw_charging_timer=%d)\n", BMT_status.charging_timer, sw_charging_timer_enable, sw_charging_timer);
}
static ssize_t store_SW_Charging_Timer_Status(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	if (2 == sscanf(buf, "%d %d", &sw_charging_timer_enable, &sw_charging_timer)) {
		battery_xlog_printk(BAT_LOG_CRTI, "[EM] sw_charging_timer_enable = %d, sw_charging_timer = %d\n", sw_charging_timer_enable, sw_charging_timer);
	} else {
		battery_xlog_printk(BAT_LOG_CRTI, "[EM] format error!\n");
	}
    return size;
}
static DEVICE_ATTR(SW_Charging_Timer_Status, 0664, show_SW_Charging_Timer_Status, store_SW_Charging_Timer_Status);
/*[Lavender][bozhi_lin] 20150415 end*/
// <<< 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
static ssize_t show_SW_Enable_llk(struct device *dev,struct device_attribute *attr,char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "show_SW_Enable_llk:%d\n", BMT_status.enable_llk);
    return sprintf(buf, "%d\n", BMT_status.enable_llk);
}
static ssize_t store_SW_Enable_llk(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
		sscanf(buf, "%d", &BMT_status.enable_llk);
	  battery_xlog_printk(BAT_LOG_CRTI, "store_SW_Enable_llk = %d\n", BMT_status.enable_llk);
	  return size;
}
static DEVICE_ATTR(SW_Enable_llk, 0664, show_SW_Enable_llk, store_SW_Enable_llk);

static ssize_t show_SW_llk_socmax(struct device *dev,struct device_attribute *attr,char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "show_SW_llk_socmax:%d\n", BMT_status.llk_socmax);
    return sprintf(buf, "%d\n", BMT_status.llk_socmax);
}
static ssize_t store_SW_llk_socmax(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
		sscanf(buf, "%d", &BMT_status.llk_socmax);
	  battery_xlog_printk(BAT_LOG_CRTI, "store_SW_llk_socmax = %d\n", BMT_status.llk_socmax);
	  return size;
}
static DEVICE_ATTR(SW_llk_socmax, 0664, show_SW_llk_socmax, store_SW_llk_socmax);

static ssize_t show_SW_llk_socmin(struct device *dev,struct device_attribute *attr,char *buf)
{
    battery_xlog_printk(BAT_LOG_CRTI, "show_SW_llk_socmin:%d\n", BMT_status.llk_socmin);
    return sprintf(buf, "%d\n", BMT_status.llk_socmin);
}
static ssize_t store_SW_llk_socmin(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
		sscanf(buf, "%d", &BMT_status.llk_socmin);
	  battery_xlog_printk(BAT_LOG_CRTI, "store_SW_llk_socmin = %d\n", BMT_status.llk_socmin);
	  return size;
}
static DEVICE_ATTR(SW_llk_socmin, 0664, show_SW_llk_socmin, store_SW_llk_socmin);
// >>> 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
...
#endif
        // <<< 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
        bat_data->enable_llk= BMT_status.enable_llk;
        bat_data->llk_socmax= BMT_status.llk_socmax;    /* API */
        bat_data->llk_socmin= BMT_status.llk_socmin;
        // >>> 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
}
...
#if 1
			if (BMT_status.bat_charging_state == CHR_TIMEOUT) {
...
//<2015/03/30-tedwu, Update for new charge algorithm.
	if (BMT_status.temperature > TEMP_POS_60_THRESHOLD) {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[B]%s(%d): [BATTERY] Battery Over high Temperature(%d) !!\n\r",
				    __func__, __LINE__, TEMP_POS_60_THRESHOLD);
		
		g_batt_temp_status  = TEMP_POS_HOT;
		return  PMU_STATUS_FAIL;
	} else if (BMT_status.temperature > TEMP_POS_45_THRESHOLD)	 {
		if ((g_batt_temp_status == TEMP_POS_HOT)
			&& (BMT_status.temperature > TEMP_POS_60_THRES_MINUS_TOLERENRCE)) {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_HOT to TEMP_POS_WARM\n\r");
		} else {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] Battery Temperature between %d and %d !!\n\r",
					    TEMP_POS_45_THRESHOLD, TEMP_POS_60_THRESHOLD);

			g_batt_temp_status = TEMP_POS_WARM;
			if (BMT_status.bat_charging_state == CHR_ERROR){
	      		BMT_status.bat_charging_state = CHR_PRE;
			}
		}
	} else if (BMT_status.temperature >= TEMP_POS_10_THRESHOLD) {
		if ((g_batt_temp_status == TEMP_POS_WARM)
			&& (BMT_status.temperature > TEMP_POS_45_THRES_MINUS_TOLERENRCE)) {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_WARM to TEMP_POS_NORMAL\n\r");
		} else if ((g_batt_temp_status == TEMP_POS_COOL)
			&& (BMT_status.temperature < TEMP_POS_10_THRES_PLUS_TOLERENRCE)) {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_COOL to TEMP_POS_NORMAL\n\r");
		} else {
			battery_xlog_printk(BAT_LOG_CRTI,
		...
		if ((g_batt_temp_status == TEMP_POS_COLD)
			&& (BMT_status.temperature < TEMP_POS_0_THRES_PLUS_TOLERENRCE)) {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_COLD to TEMP_POS_COOL\n\r");
		} else {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] Battery Temperature between %d and %d !!\n\r",
					    TEMP_POS_0_THRESHOLD, TEMP_POS_10_THRESHOLD);

			g_batt_temp_status = TEMP_POS_COOL;
			if (BMT_status.bat_charging_state == CHR_ERROR){
	      		BMT_status.bat_charging_state = CHR_PRE;
			}
		}
		...
		g_batt_temp_status = TEMP_POS_COLD;
		return PMU_STATUS_FAIL;
	}
//>2015/03/30-tedwu
#elif defined(LAVENDER)
//	battery_xlog_printk(BAT_LOG_CRTI,
//				    "[B]%s(%d): [BATTERY] BMT_status.temperature = %d, g_batt_temp_status=%d, BMT_status.bat_charging_state=%d\n\r",
//				    __func__, __LINE__, BMT_status.temperature, g_batt_temp_status, BMT_status.bat_charging_state);
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if ((g_batt_temp_status == TEMP_POS_HOT)
			...
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_HOT to TEMP_POS_WARM\n\r");
		} else if ((g_batt_temp_status == TEMP_POS_NORMAL)
			&& (BMT_status.temperature <= TEMP_POS_45_THRES_PLUS_TOLERENRCE)) {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_NORMAL to TEMP_POS_WARM\n\r");
		} else {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] Battery Temperature between %d and %d !!\n\r",
					    TEMP_POS_45_THRESHOLD, TEMP_POS_60_THRESHOLD);
		...
		if ((g_batt_temp_status == TEMP_POS_NORMAL)
			&& (BMT_status.temperature >= TEMP_POS_10_THRES_MINUS_TOLERENRCE)) {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_NORMAL to TEMP_POS_COOL\n\r");
		} else if ((g_batt_temp_status == TEMP_POS_COLD)
			&& (BMT_status.temperature <= TEMP_POS_0_THRES_PLUS_TOLERENRCE)) {
			battery_xlog_printk(BAT_LOG_CRTI,
					    "[BATTERY] g_batt_temp_status can't change from TEMP_POS_COLD to TEMP_POS_COOL\n\r");
		} else {
  ...
  else if( g_batt_temp_status == TEMP_POS_LOW )
  {
	...
	if (BMT_status.temperature > MAX_CHARGE_TEMPERATURE) {
		...
		if( BMT_status.temperature <= MAX_CHARGE_TEMPERATURE_MINUS_X_DEGREE ) {
			...
			//return PMU_STATUS_OK;
			#else
            g_batt_temp_status = TEMP_POS_DEC_CHARGE; //<2015/02/03-tedwu,//>
            return PMU_STATUS_FAIL; 
            #endif
		}
  #else
...
/*[Lavender][bozhi_lin] set battery initial soc to 1 for avoid user confused 20150612 begin*/
	if ((previous_SOC == -1) && (BMT_status.SOC <= 1)  && (BMT_status.UI_SOC == 0)) {
		BMT_status.UI_SOC = 1;
	}
/*[Lavender][bozhi_lin] 20150612 end*/
	batteryIndex++;
...
/*[Lavender][bozhi_lin] set AC timer to 5 hours and USB to 100 hours 20150415 begin*/
#if 1
	if (sw_charging_timer_enable) {
		BMT_status.charging_timer = sw_charging_timer;
	}

	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] BMT_status.total_charging_time = %d, BMT_status.charging_timer = %d\n", BMT_status.total_charging_time, BMT_status.charging_timer);
	
	if (g_call_state == CALL_ACTIVE) {
		BMT_status.total_charging_time = 0;
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] during call set BMT_status.total_charging_time = %d\n", BMT_status.total_charging_time);
	}
	// <<< 2016/05/27-dangyiwang. Battery. add adb command for close Safety timer
    UINT32 totalChargerTime = 0;
    totalChargerTime = BMT_status.total_charging_time;
    if(g_safety_timer_enable_flag)
        totalChargerTime = BMT_status.total_charging_time;
    else
        totalChargerTime = 0;
	/* Charging OT */
	if (totalChargerTime >= BMT_status.charging_timer) {
	//if (BMT_status.total_charging_time >= BMT_status.charging_timer) {
	// >>> 2016/05/27-dangyiwang. Battery. add adb command for close Safety timer
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Charging Over Time.\n");
		//<2015/11/11-JackHu, check voltage to determine add charging timer fix DMS06428196.
		/*[Lavender][bozhi_lin] add voltage check to avoid trigger charging timeout 20150708 begin*/
		if (BMT_status.bat_vol <= CHARGING_TIMEOUT_ACTIVE_PROTECT_VOLTAGE) {
			BMT_status.total_charging_time = BMT_status.total_charging_time - CHARGING_TIMEOUT_COMPENSATION_TIME;
			if(BMT_status.total_charging_time<=0)
			    BMT_status.total_charging_time=0;
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] approach max voltage decrease charging time, BMT_status.total_charging_time = %d\n", BMT_status.total_charging_time);
		} else {
			status = PMU_STATUS_FAIL;
		}
...
/*[Lavender][bozhi_lin] set maximun voltage to 4.2V during call 20150415 begin*/
	...
	battery_xlog_printk(BAT_LOG_FULL, "CheckCall State:%d, bat_vol:%d\n", g_call_state, BMT_status.bat_vol);
	//>2015/02/02-tedwu
...
/*[Lavender][bozhi_lin] set AC timer to 5 hours and USB to 100 hours 20150415 begin*/
#if 1
		BMT_status.bat_charging_state = CHR_TIMEOUT;
#else
		BMT_status.bat_charging_state = CHR_ERROR;
#endif
/*[Lavender][bozhi_lin] 20150415 end*/
...
//<2015/01/18-tedwu, For battery over temperature protection.
	...
	if(BMT_status.temperature > TEMP_POS_CRITICAL_THRESHOLD)  //<2014/05/18-tedwu //>
    #else
	//if(BMT_status.temperature > TEMP_POS_60_THRESHOLD) //before
	if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)//mr1 codebase
    #endif
...
#else
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
        // <<< 2016/07/01-youchihwang, Battery. MTK [FAQ15039]. Checking charger type again avoid detecting error.
...
static void mt_battery_charger_detect_check(void)
{
	...
	BMT_status.charger_vol = battery_meter_get_charger_voltage();
		...
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
		    (DISO_data.diso_state.cur_vusb_state == DISO_ONLINE)) {
#endif
                        battery_log(BAT_LOG_CRTI, "first check charger type\n");
			mt_charger_type_detection();
                        // <<< 2016/07/01-youchihwang, Battery. MTK [FAQ15039]. Checking charger type again avoid detecting error.
                        // <<< 2016/07/01-youchihwang, Battery. MTK [FAQ15039]. Checking charger type more times
                        if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
                                battery_log(BAT_LOG_CRTI, "second check charger type\n");
                                mt_charger_type_detection();
                                if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
                                        battery_log(BAT_LOG_CRTI, "third check charger type\n");
                                        mt_charger_type_detection();
                                        if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
                                                battery_log(BAT_LOG_CRTI, "fourth check charger type\n");
                                                mt_charger_type_detection();
                                                if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
                                                        battery_log(BAT_LOG_CRTI, "fifth check charger type\n");
                                                        mt_charger_type_detection();
                                                }
                                        }
                                }
...
// <<< 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
void battery_charging_sepecial_stop_charge(void)
{
    static unsigned int bf_llk_status = 0;
	battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]BMT_status.enable_llk=%d \n",BMT_status.enable_llk);
	if(BMT_status.enable_llk==1)
	{
	    bf_llk_status=1;
		battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]BMT_status.llk_socmax=%d ,  BMT_status.llk_socmin:%d\n",BMT_status.llk_socmax, BMT_status.llk_socmin);
	 	if(BMT_status.UI_SOC2 >= BMT_status.llk_socmax)
	    {
			battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]charging_enable=false\n");
			cmd_discharging=1;
		}else if(BMT_status.UI_SOC2 <= BMT_status.llk_socmin)
		{
			battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]charging_enable=true\n");
			if(cmd_discharging==1)
			{
			    cmd_discharging=0;
				 battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]cmd_discharging==1\n");
			}
		}
	}else
	{
	    if(bf_llk_status==1)
		{
			...
			{
			    cmd_discharging=0;
				battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]bf_llk_status==1 cmd_discharging==1\n");
			}else
			    battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]bf_llk_status==1 cmd_discharging==0\n");
		}else
	        battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]BMT_status.enable_llk==0 nothing \n");
		bf_llk_status=0;
	}
	battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]cmd_discharging:%d \n",cmd_discharging);
	battery_xlog_printk(BAT_LOG_CRTI,"[BATTERY]BMT_status.bat_charging_state:%d \n",BMT_status.bat_charging_state);
	return;
}
// >>> 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
...
void BAT_thread(void)
{
	...
	if (BMT_status.charger_exist == KAL_TRUE) {
                // <<< 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
                battery_charging_sepecial_stop_charge();
                // >>> 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
...
static ssize_t show_safety_timer_off(struct device *dev, struct device_attribute *attr, char *buf)
{
    g_safety_timer_enable_flag = KAL_FALSE;
    return 0;
}
static ssize_t store_safety_timer_off(struct device *dev, struct device_attribute *attr, char *buf)
{
    g_safety_timer_enable_flag = KAL_FALSE;
    return 0;
}
...
static ssize_t show_safety_timer_on(struct device *dev, struct device_attribute *attr, char *buf)
{
    g_safety_timer_enable_flag = KAL_TRUE;
    return 0;
}
static ssize_t store_safety_timer_on(struct device *dev, struct device_attribute *attr, char *buf)
{
    g_safety_timer_enable_flag = KAL_TRUE;
    return 0;
}
...
static DEVICE_ATTR(safety_timer_off, 0664, store_safety_timer_off, show_safety_timer_off);
static DEVICE_ATTR(safety_timer_on, 0664, store_safety_timer_on, show_safety_timer_on);

static int safety_timer_create_attr(struct device *dev) 
{
	int err = 0;
	...
	printk("[Battery_probe] create_safety_timer_attr");  
    err = device_create_file(dev, &dev_attr_safety_timer_off);
    err = device_create_file(dev, &dev_attr_safety_timer_on);
	return err;
}
// >>> 2016/05/27-dangyiwang. Battery. add adb command for close Safety timer
/* ///////////////////////////////////////////////////////////////////////////////////////// */
...
void charger_hv_detect_sw_workaround_init(void)
{
	...
	//<2015/02/21-tedwu, [DMS06234677][DMS06327830]Fix boot logo repeat in KPOC.
	if ((upmu_is_chr_det() == KAL_TRUE)) {
		check_battery_exist();
	}
	//>2015/02/21-tedwu
...
/*[Lavender][bozhi_lin] set AC timer to 5 hours and USB to 100 hours 20150415 begin*/
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_SW_Charging_Timer_Status);
/*[Lavender][bozhi_lin] 20150415 end*/
#if defined(CONFIG_MTK_PUMP_EXPRESS_SUPPORT) || defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Pump_Express);
#endif
        // <<< 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_SW_Enable_llk);
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_SW_llk_socmax);
        ret_device_file = device_create_file(&(dev->dev), &dev_attr_SW_llk_socmin);
        // >>> 2016/04/01-youchihwang. Battery. FP022589. Battery Swelling Mitigation for retail demo.
	}
...
/*[Lavender][bozhi_lin] set maximun voltage to 4.2V during call 20150415 begin*/
static ssize_t callstate_cmd_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int len = 0, bat_call_state = 0;
	char desc[32];
	...
	if (sscanf(desc, "%d", &bat_call_state) == 1) {
		g_call_state = bat_call_state;

		battery_xlog_printk(BAT_LOG_CRTI, "g_call_state=%d\n", g_call_state);

		return count;
	} else {
		battery_xlog_printk(BAT_LOG_CRTI,
				    "  bad argument, echo [bat_call_state] > callstate_cmd\n");
	}

	return -EINVAL;
}

static int proc_callstate_show(struct seq_file *m, void *v)
{
	seq_printf(m, "g_call_state=%d\n", g_call_state);

	return 0;
}

static int proc_callstate_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_callstate_show, NULL);
}

static const struct file_operations callstate_cmd_proc_fops = {
	.open = proc_callstate_open,
	.read = seq_read,
	.write = callstate_cmd_write,
};
/*[Lavender][bozhi_lin] 20150415 end*/
...
/*[Lavender][bozhi_lin] set maximun voltage to 4.2V during call 20150415 begin*/
		proc_create("callstate_cmd", S_IRUGO | S_IWUSR | S_IWGRP, battery_dir, &callstate_cmd_proc_fops);
