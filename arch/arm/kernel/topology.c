...
static void __init parse_dt_topology(void)
{
	...
	for (cpu = 0; cpu < nr_cpu_ids; cpu++) {
		...
		if (cpu < 4)
			...
			reg = 0x100 | (cpu-4);
		reg = reg | cpu;
		cpu_capacity[cpu].capacity = 1024;
		cpu_topology[cpu].core_id = reg & 0xFF;
		cpu_topology[cpu].socket_id = (reg >> 0x8) & 0xFF;
		pr_debug("CPU%u: hwid(0x%0x) socket_id(%i) core_id(%i)\n",
				 cpu, be32_to_cpup(&reg), cpu_topology[cpu].socket_id, cpu_topology[cpu].core_id);
		cpu_capacity[cpu].hwid = reg;
	}

	min_capacity = max_capacity = (unsigned long) (1024);
}
